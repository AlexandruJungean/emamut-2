...
export async function generateMetadata({ params }: TermsPageProps): Promise<Metadata> {
  ...
  return {
    title: locale === 'ro' ? 'Termeni și Condiții - Emamut' : 'Terms & Conditions - Emamut',
  }
}

async function getTermsContent(locale: string): Promise<string> {
  try {
    const filePath = path.join(process.cwd(), 'content', 'legal', locale, 'termeni-conditii.md')
    const content = await fs.readFile(filePath, 'utf-8')
    return content
  } catch {
    // Fallback to Romanian if translation doesn't exist
    try {
      const filePath = path.join(process.cwd(), 'content', 'legal', 'ro', 'termeni-conditii.md')
      const content = await fs.readFile(filePath, 'utf-8')
      return content
    } catch {
